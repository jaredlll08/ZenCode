...
import { ExtensionContext, window, workspace, languages, TextDocument, TextEdit } from 'vscode';
...
export function activate(context: ExtensionContext) {
	...
	getJavaHome().then(javahome => {
		...
		const classpath: string = join(context.extensionPath, "../", "build", "libs", "LanguageServerProvider-0.3.8-all.jar")
		const args: string[] = ['-cp', classpath];
		const main = 'org.openzen.zenscript.StandardIOLauncher';
		let debug = '-agentlib:jdwp=transport=dt_socket,server=n,address=65535,suspend=y';
