package org.openzen.zenscript.formatting;
...
@SuppressWarnings({"unused", "StatementWithEmptyBody", "DuplicateBranchesInSwitch"})
public class FormattingParser {


	private final List<String> semanticTokens = new ArrayList<>();
		...
		switch (next.getType()) {
			...
			case K_VAL:
				...
				if (tokens.isNext(T_IDENTIFIER)) {
					ZSToken name = tokens.next();
					variables.add(name.content);
					token(name);
				}
	...
	private void token(ZSTokenParser tokens) throws ParseException {
		semanticTokens.add(tokens.next().content);
	}

	private void token(ZSToken token) {
		semanticTokens.add(token.content);
	}
